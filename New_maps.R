...
regiony <- read_sf(r'(../../../../OneDrive - MUNI/2022_Expanzky/vyroba_seznamu/Regiony_shp/Regiony_wgs2.shp)')
...
for (i in loop_over) {
  ...
  ls[[i]] <- regiony |>
    ...
    scale_fill_manual(breaks = c(FALSE, TRUE), values = c('grey88', '#F49E19'), drop = F) +
    theme_void() +
    labs(title = i,
         x = '', y = '') +
