...
habitat <- read_xlsx(r'(C:/Users/krystof/OneDrive - MUNI/2022_Expanzky/expanzni-druhy-cz-biotopy-2023-09-27.xlsx)')

habitat |> colnames()

habitat |>
  select(species:`Anthropogenic woodlands, clearings`) |>
  pivot_longer(-1) |>
  filter(value == 1) |>
  group_by(name) |>
  count() |>
...
ggsave('figures\\Figure_7.png', width = 8, height = 8)