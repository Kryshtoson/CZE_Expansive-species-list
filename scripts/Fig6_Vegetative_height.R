...
traity <- read_xlsx(r'(C:/Users/chytryk/OneDrive - MUNI/2022_Expanzky/expanzky_traity_2023-09-27.xlsx)')
order <- c('Tree', 'Shrub', 'Chamaephyte', 'Hemicryptophyte', 'Geophyte', 'Hydrophyte', 'Therophyte')

bind_rows(traity |> filter(origin == 'native' & expansive == 'no'),
          traity |>
...
step |> ggplot(aes(expansive, height)) +
  ...
  geom_boxplot(data = step |>
    filter(!(expansive == 'Expansive\nflora' & name %in% c('Hydrophyte', 'Chamaephyte'))),
